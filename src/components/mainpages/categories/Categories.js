...
function Categories() {
    ...
    try {
      if (onEdit) {
        const updateCat = await axios.put(
          `https://webdoanbe.onrender.com/api/category/${id}`,
          { name: category },
          {
        ...
        const createCat = await axios.post(
          "https://webdoanbe.onrender.com/api/category",
          { name: category },
          {
            headers: { Authorization: token },
          }
  ...
  const deleteCategory = async (id) => {
    try {
      const deleteCat = await axios.delete(`https://webdoanbe.onrender.com/api/category/${id}`, {
        headers: { Authorization: token },
      });
      Swal.fire({
        position: "center",
